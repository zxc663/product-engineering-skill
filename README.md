...
**定位边界（2026-09-23 裁决）**：本仓库=设计部；包本体=**交互工程**（Product Contracts 规范层的**第一垂直域**），母架构其余域（Intent/Structure/Runtime 等）**均未实现**——详见 [docs/product-architecture.md](docs/product-architecture.md)。
...
| 风险自适应四档 | L0 不启动 / L1 轻量 / L2 六问+trace / L3 +recovery+evidence+regression | 同上（判定表 §风险档位） |
