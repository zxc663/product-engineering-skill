...
- **图性质门禁挡不住「契约承诺的多重出路被削」**（2026-09-23 子代理试验实测 + 本会话独立复现）
  ...
  - **复核加深（2026-09-23 试验 settle 后，主代理三变异实测）**：不止转换级——**引用完整性同样未查**：删掉整个错误态、只留悬空入边（`exporting--EXPORT_FAIL-->` 指向不存在的态），门禁仍 `exit=0`；对照组 exit=0／抽一条转换 exit=0／抽全部出边 **exit=1（C2+C4 双报）**＝门禁在其设计域内有效。两条缺口 → 候选 12（recovery 行↔转换对账）+ 候选 13（target ∈ states，一行级）。
- **新编号体系入场前必须全库同形扫描**（2026-09-23 修正三落地时自查发现）
  - 症状：用户层栈 L0-L10 与本体既有「风险自适应四档 L0-L3」完全同形，工作流判级 L1/L2-S/L2-F/L3 是第三套——同一份文档里 `L2` 可指「层级/档位/判级」三义，失忆的下一会话必然误读。
...
### 2026-09-23 04:40｜闭环试验 settle：结论交代 + 三变异独立复核 + 报告/候选 12/13 落位（清 04:29 会话待办）
- 试验终态（dwfrun-aaaaf612 完成通知）：**正向十环成立**（11 态/26 转换/双门禁 exit=0/首跑 spec-trace 报 127 项 T4 后修复/账本 7 行/上游 8 条待真人复核）；**核心负结论「缺失可检出」未获实证**（抽 `export_failed --RETRY_EXPORT--> exporting` 后 exit=0）
- **主代理三变异【实测】**（临时副本，试验仓零改动）：对照组 exit=0 ｜ 变体 A 抽一条转换 exit=0＝复现负结论 ｜ 变体 B 抽全部出边 **exit=1（C2+C4 双报＝门禁设计域内有效）** ｜ **变体 C 删整个错误态留悬空入边 exit=0＝新缺口：引用完整性不检出**（报告未覆盖）
- 报告自身独立复核 18 条有效：命令原文不可重放／注入无 diff 留证／时间线停在 `4182576` 而脚本收尾提交 `bbcea84`
- **落位**：结论档 docs/closed-loop-report.md（结论摘要+两条判据缺口+下一步五项+Q1-Q8 交接）；direction §十一 遗留① 更新（已实跑+结论+下一步）；本体 §4 登记**候选 12**（recovery 行↔转换双向对账，验收=变体 A）与**候选 13**（target ∈ states 引用完整性，验收=变体 C）+ 三副本同步
- **多会话合并**：04:29 会话（commit `8d61d0a`）的中途核查待办（报告回写 + 候选 12 登记）本轮已完成——**勿重复**；其教训区第 3 条已由本轮**追加**「复核加深」段（不改写他会话条目）
- GATE: {level=L2-S, ev=exec+indep, v=试验 settle 结论交代+三变异独立复核+报告&候选 12/13 落位, cmd=python statechart-gate.py --file <临时副本×3 变体>（0/0/1/0）+ diff -rq 三副本 + git commit×2, exit=0, files=docs/closed-loop-report.md + docs/direction.md + memory/agent-log.md + <源库>SKILL.md（三副本同步）, refs=0(未跑 lookup，0 照报), errpath=Edit 报「文件已被修改」（04:29 会话并发写入）→按 #294 重读实文 + 合并而非覆盖；教训区用「追加」不改写他会话条目, lessons=「缺失可检出」是判据集属性——判据只到态级则转换级与引用级皆盲；负结论须自做变体梯度（A/B/C）才能定位边界；多会话共写一档必须重读+合并+声明待办归属, exempt=报告时间线较实际少一条提交（bbcea84）；浏览器级证据 1/11 态；无跨实验对照组, caps=—（本轮纯本地复核+文档；试验执行=上一轮 CreateWorkflow×6 无头子代理）, effort=试验仓五档实读（562 行）+三变异实测+报告档+本体候选 2 条+三副本同步+最近更新/教训区/流水三处合并, stop_reason=—}

### 2026-09-23 04:41｜交接：状态段归并 + 交接档（入口索引）〔时间戳校正：原记 04:55 系估时漂移，agent-log mtime 04:41:05 + commit a37171a 04:41:12 实证〕
- 触发：用户「交接」
- 状态段归并（治上下文污染）：13 行堆叠遗留（含多轮重复与已了结项：第四包本体未写／门禁脚本未产／statechart-gate 待实现，均已成）→ **1 行现役队列（13 项）+ 1 行历史留档**；符合「状态段 ≤10 行」纪律，条目去重无丢失
- 新增 `docs/HANDOVER.md`：入口索引式交接（现状一句话／必读清单／遗留队列／待用户裁决／回滚基线／多会话与本体改动 SOP），**不复制正文**防双权威
- GATE: {level=L2-S, ev=exec+indep, v=交接（状态段归并+交接档）, cmd=grep 全仓残留排查（0 命中）+ wc -l 状态段 + git commit（设计部）, exit=0, files=docs/HANDOVER.md + memory/agent-log.md, refs=0(未跑 lookup，0 照报), errpath=—, lessons=状态段的「遗留」是易腐字段——每轮追加不归并就会变成 13 行堆叠（含已了结项），交接时必须归并（现役队列+历史留档两行制）；交接档必须入口索引式（不复制正文），否则与权威档双份漂移, exempt=—, caps=—, effort=残留全仓排查+状态段 13→2 行归并+交接档 6 节+提交, stop_reason=—}

### 2026-09-23 04:43｜接交核验（新会话冷启动；用户「交接」）
- 触发：用户「交接」——上一会话已完成归并+HANDOVER.md；本轮=**接交侧验实**（防「档说一套、实物另一套」，不采信档内转述）
- **逐项验实（实物为准）**：①三副本 `diff -rq`——`.zcode` 与源库零差异；`.agents` 仅 `references/decision-ledger.md` 已知漂移（=遗留⑬，与档载一致）②三仓 HEAD 实测——设计部 `a37171a`（=基线 `cfb0599`+交接提交）／源库 `aeba388`（本地未 push）／试验仓 `bbcea84`，与 HANDOVER §五 逐格相符 ③本体 §4 候选 12·13 在册（含验收用例 A/C 与实测日期）④docs 清点齐（报告/协议/定义/母架构四档在位）⑤状态段 9 行 ≤10 达标
- **修正**：上一条时间戳 04:55 系估时漂移——agent-log mtime `04:41:05` + commit `a37171a` `04:41:12` 实证不可能是 04:55 → 校正为 04:41 并就地留注（未改其他内容）；`最近更新` 行同步刷新
- 观察（不擅自改）：04:40 条（其 commit `cfb0599` 为 04:38:22）亦轻微前漂 ~2 分钟，无实据精确定位 → 留档不改；建议写档前一律 `date` 取实时钟再落笔
- 未动：三副本漂移件（待裁）、试验仓（只读）、源库（未 push）、`.mimosa/`（工具自身 hook 状态，不入库）
- GATE: {level=L1, v=接交核验+时间戳校正, cmd=diff -rq 三副本 + git log/rev-parse 三仓 + grep 候选12/13 + grep 04:55 全仓 + sed 状态段行数, exit=0, files=memory/agent-log.md, refs=0(未跑 lookup，0 照报), errpath=—, lessons=交接要「验实」不要「复述」——上一条自身的时间戳漂移只有对实物（mtime/commit）才暴露；凭估时写档会产出未来时间戳，写档前先取实时钟, exempt=—, caps=—, effort=三副本/三仓/候选在册/docs/状态段五项验实+两处时间戳修正+一条核验记录, stop_reason=—}

### 2026-09-23 04:52｜修正三：层级栈 L0-L10 与层级门（用户定稿——方向控制）+ 命名冲突处置
- 触发：用户给出 11 层栈（L0 产品目标…L10 验证）+「最重要的能力之一不是告诉模型正确答案，而是**强迫模型先确认自己正在回答哪一层的问题**」+ 四例动作级追问；铁律=**实现层完整性≠产品工程完整性**
- **差异分析**（防重复劳动）：既有资产已覆盖层栈中后段（L1-L4=product-object 六问+J 域；L6-L7=interaction-bridge/statechart；L10=spec-trace/取证）→ 真增量三件：①显式层栈坐标系（含 L0 只检查不生产/L5 未实现声明）②**层级门三段式**（定位→检上游→放行/回补；自答优先不打断用户；四条纪律=不得越级下沉/同层不阻塞/L0只检查不生产/反向回补）③**触发拦截表 10 行**（四例泛化：按钮位置→L8 需 L2/L3 已确认；EmptyState→空态产品语义；新建组件→同职责存在性；新建接口→谁管理/来源/后台）
- **本体**（源库 c95251a，三副本同步）：新增 `references/layer-stack.md`（63 行）；SKILL.md 跑道步骤 0 重写=0a 层级门+0b 六问；判定表 +**K 域四条**（v0.5·34→v0.6·38）；anti-excuses #11+「层错位触发」类；capability-map 加「开工前·层级门」行；contract-schema/interaction-bridge 加层级门上游指针；登记**候选 14**（层级声明与上游引用存在性）
- **命名处置（自查发现的三义冲突）**：保用户命名（层=裸 L+数字）／改自造命名（风险四档 L0-L3→**档0-档3**，6 处）／判级写全称——教训区第 4 条
...
### 2026-09-23 07:41｜夜班追加段：product-object-gate 落地转正＋A/B 口径草案＋修复后扩样本 3/3（收口）
- **product-object-gate P1-P4**（候选 11/14＋主功能唯一性计数一次落地）：P1 定义存在性（职责句禁展示/显示=J-29）｜P2 主功能唯一（核心任务数≠1→exit 1）｜P3 层级声明+上游引用存在性（L1-L4 缺层/file 悬空逐层报）｜P4 可运营性（J-32）；schema=product-object.md §6；selftest 两态＋试验仓语料转写（正 0/负 1）＋**真实项目首用**——甲仓（修正二前产物）忠实转写被检出 10 项真实缺口→**候选 11/14 转正**（源库 0e1398a/52bc031）
- **A/B 四指标口径草案** docs/ab-four-metrics.md（defect_escape 判定程序与 escape 定义＝独立评审+主会话复现+自报不算／coverage 可数清单／可证伪判据）——待追认
- **修复后条件扩样本 3/3**：N6d 投票（档3，双门禁独立复跑绿，vm 桩 27 断言）＋N6e 书签（**档2**，CDP 39 断言，statechart 复跑绿）——跨三任务闭环成立；档位边界判例分歧（删除可逆：档2vs档3）入档作校准数据（cold-start-report 扩样本节，4b2eafa）
- GATE: {level=L2-F, ev=exec+cover, v=追加段（gate 转正/口径草案/扩样本终判）, cmd=product-object-gate --selftest（rc=0）+ --file 语料正负变体（0/1）+ 甲仓转写（rc=1 检出 10 项）+ N6d/N6e statechart 独立复跑（rc=0）, exit=0, files=<源库>scripts/product-object-gate.py+references/{product-object,layer-stack}.md+SKILL.md；<设计部>docs/{real-project/toolbox.object.json,ab-four-metrics.md,cold-start-report.md,HANDOVER.md}+memory/agent-log.md, refs=0(未跑 lookup，0 照报), errpath=mktemp 的 /tmp 对 Windows Python 不可见→显式 Windows 临时路径；sed -i 绕过工具读态致 HANDOVER Edit 报 modified→重读实文再改（#294）；本轮 Edit 误以新条目替换 06:45 条目标题→读尾部实文发现→两步还原+移至文件末尾（结构错误自查自纠）, lessons=「忠实转写、留空未答」的真项目语料比合成负例更能证明门禁价值（检出的全是真缺口）；档位矩阵边界需判例积累；追加条目必须锚定文件真尾而非前条标题, exempt=N6d 真浏览器未做（vm 桩代）；A/B 口径待追认；L0/L5 空白未动, caps=Agent×2（N6d/N6e）, effort=一门禁全链+两样本终判+三档刷新+四轮推送+结构错误自纠, stop_reason=—}

### 2026-09-23 18:01｜README 门面重构：问题定义主叙事上脸（用户供稿）+推送两波折
- 触发：用户供问题定义全文（判据无人区一句话/成本结构镜像颠倒表/三层收敛/衰减链五级/答案与边界/核心句），令「重构并更新并推送 Git 门面说明文档」
