...
## 四、待用户裁决（阻塞项）——**已全部批复（2026-09-25 00:05，用户预授权「按默认走」）**

- ~~`docs/ruling-package.md` 全部~~ → **已批复落地**：Q1-Q8 全同意默认；AB 四指标追认（口径转正）；账本=合并入源库（e4f4fae，diff -rq 核验一致）；D1/D2 追认。批复记录=速裁包文末。
- ~~F3 双目录去重方向~~ → **裁决=以 .agents 为准**；物理去重涉删除，按白名单制挂起待用户在场执行。
- （非阻塞·已执行待追认→已追认）档0-档3 改名；触发词第三次密集化；v0.2.2 description 重构（A/B 依据在档）；v0.2.3 反借口 #12 与档位判例。
- **现役待办转移**：夜战计划档 `docs/night-battle-plan-20260925.md`（缺口清单 G1-G10+E 批实验序列）。
